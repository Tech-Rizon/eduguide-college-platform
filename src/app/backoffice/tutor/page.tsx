...
export default function TutorDashboardPage() {
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error("Failed to load tutor queue.");
        return;
      }
  ...
  return (
    ...
    >
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle>Assigned Tutoring Tickets</CardTitle>
          <CardDescription className="text-slate-400">
            Auto-assignment routes new tutoring requests to tutor queues.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {tickets.length === 0 && (
            <p className="text-sm text-slate-400">No tutoring tickets are currently assigned to you.</p>
          )}
        </CardContent>
      </Card>
    </BackofficeShell>
